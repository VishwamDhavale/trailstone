...
function selfcheck() {
  ...
  { // Reversal mid-work (reversal-midwork eval): the edit hook re-fires when a file's rules change, and
    ...
    for (const [cwd, what] of [[other, "another repo"], [bare, "no repo"]]) {
      ...
      const out = hx({ hook_event_name: "PreToolUse", tool_name: "Edit", tool_input: { file_path: join(d5, "src", "a.ts") } });
      let ex = out; try { ex = JSON.parse(out).hookSpecificOutput.additionalContext; } catch {} // raw JSON escapes Windows backslashes
      ok(/timestamps are/.test(ex) && ex.includes(join(d5, "src", "a.ts")), `edit hook follows the file's repo, not the session's (${what}), and names it by full path (got: ${JSON.stringify(ex.slice(0, 300))})`);
      const id = `d_x${sx}`;
